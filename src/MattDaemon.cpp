...
void MattDaemon::loop() {
  set_sighandler();
  while (running) {
    TintinReporter::get_instance().info("Starting poll");
    int result = poll(_poll_fds.data(), _poll_fds.size(), -1);
    TintinReporter::get_instance().debug("poll returns=" +
...
void MattDaemon::handle_client_fd(pollfd poll_fd) {
  ...
  if (poll_fd.revents & POLLIN) {
    ...
    TintinReporter::get_instance().info("fd=" + std::to_string(poll_fd.fd) +
                                        ": " + msg);
  } else {
    remove_poll_fd(poll_fd.fd, true);
  }
...
void MattDaemon::handle_client_connection(pollfd pollfd) {
  ...
  int client_fd = accept(pollfd.fd, nullptr, nullptr);
  if (client_fd < 0) {
...
void MattDaemon::add_poll_fd(pollfd fd) {
  ...
  if (it != _poll_fds.end()) {
    TintinReporter::get_instance().warn(
        "add_poll_fd: fd=" + std::to_string(fd.fd) + " already in _poll_fds");
  }
...
static void signal_handler(int sig) {
  ...
  if (sig == SIGINT || sig == SIGQUIT || sig == SIGILL || sig == SIGABRT ||
      sig == SIGFPE || sig == SIGSEGV || sig == SIGPIPE || sig == SIGALRM ||
      sig == SIGTERM) {
    running = 0;
  }
}